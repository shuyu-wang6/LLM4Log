...
class Exp_Imputation(Exp_Basic):
    ...
    def gen(self, setting, test=0):
        test_data, test_loader = self._get_data(flag='gen')
        if test:
            print('loading model')
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y,batch_mask,eval_mask) in tqdm(enumerate(test_loader)):
                ...
                # random mask
                B, T, N = batch_x.shape
                batch_mask = batch_mask + eval_mask
                eval_mask = batch_mask
        ...
        '''
        trues1 = test_data.inverse_transform(trues.reshape(-1,N))

       # batch_x_copy = torch.tensor(batch_x_copy).float().to(self.device)
        trues1 = trues1.reshape(-1,T,N)

        preds1 = test_data.inverse_transform(preds.reshape(-1,N))
        ...
        f = open("new_result_imputation.txt", 'a')
        f.write(setting + "  \n")
        ...
        np.save(folder_path + 'g_pred.npy', preds1)
        np.save(folder_path + 'g_true.npy', trues1)
        return
    ...
    def test(self, setting, test=0):
       ...
       # batch_x_copy = torch.tensor(batch_x_copy).float().to(self.device)
        ...
        raw = pd.read_csv(
            os.path.join(self.args.root_path, self.args.data_path),
            encoding="utf-8-sig"
        ).values
        #raw = np.loadtxt(os.path.join(self.args.root_path, self.args.data_path), dtype=float,delimiter=",")  
        data, norm_parameters = normalization(raw)
