...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.weight_layer = nn.Linear(configs.c_out * 3, configs.c_out )

        self.mlp = nn.Sequential(
            nn.Linear(configs.d_model * 2, configs.c_out  *2),
            nn.ReLU(),
            nn.Linear(configs.c_out*2 , configs.c_out ),
        )

        self.smlp = nn.Sequential(
            nn.Linear(configs.c_out * 2, configs.c_out *2 ),
            nn.ReLU(),
            nn.Linear(configs.c_out *2 , configs.c_out ),
        )
    ...
    def imputation(self, x_enc, mask):
        ...
        #x_m_enc = self.enc_embedding(x_m_enc, None)  # [B,T,C]
        ...
        #re_x_m_enc = torch.flip(x_enc, dims = (0,1))
        re_x_m_enc = self.in_layer(re_x_m_enc)
        #re_x_m_enc = self.enc_embedding(re_x_m_enc, None)  # [B,T,C]
        ...
        tem_out = self.ln_proj(x_m_output)
        tem_out = self.out_layer(tem_out)
        #tem_out = self.mlp(tem_out)

        
        #spatial
        x_s_enc = rearrange(x_enc, 'b l m -> b m l')
        ...
        dec_out = torch.cat([x_enc,tem_out, spa_out],dim = 2)
        dec_out = self.weight_layer(dec_out)
        #dec_out = self.smlp(dec_out)
